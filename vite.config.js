...
import handleVisualEditor from './vite-plugin-handle-visual-editor';
...
export default defineConfig({
	customLogger: logger,
	define: {
		// Ekspos variabel lingkungan ke aplikasi
		'import.meta.env.EMAIL_HOST': JSON.stringify(process.env.VITE_EMAIL_HOST || ''),
		'import.meta.env.EMAIL_PORT': JSON.stringify(process.env.VITE_EMAIL_PORT || ''),
		'import.meta.env.EMAIL_USER': JSON.stringify(process.env.VITE_EMAIL_USER || ''),
		'import.meta.env.EMAIL_PASSWORD': JSON.stringify(process.env.VITE_EMAIL_PASSWORD || ''),
	},
	plugins: [
    ...
    // Only add handleVisualEditor in development
    ...(isDev ? [handleVisualEditor()] : [])
	],
	server: {
		cors: true,
		headers: {
		...
		// Handle 404s for visual editor plugin
		proxy: {
			'/dashboard/plugins': {
				target: 'http://localhost:5173',
				changeOrigin: true,
				rewrite: (path) => path.replace(/^\/dashboard\/plugins/, '')
			}
		}
	},
	resolve: {
